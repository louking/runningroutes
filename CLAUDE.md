...
The dev site is reached through the Caddy reverse proxy at `https://routes.localhost`, not the raw `docker compose` published port (`8005` per `docker-compose.dev.yml`/`.env`) — curling or otherwise probing the raw port directly returns nothing useful from outside the compose network. Same pattern as the other FSRC apps.
...
- **`rjsmin` filter**: `assets.py` specifies `filters='rjsmin'` for JS bundles, which requires the `rjsmin` Python package (distinct from `jsmin`). If `rjsmin` is not installed, `ASSETS_DEBUG=False` cannot rebuild bundles and `ASSETS_DEBUG=True` skips the filter entirely (files are concatenated raw, exposing any missing-semicolon issues). The pre-built `gen/admin.js` in the repo was minified with `rjsmin`; keep `rjsmin` in `requirements.txt`. **Don't judge bundle freshness by the presence/absence of comments** when reading a `gen/*.js` file in DevTools Sources — `rjsmin` strips *all* comments unconditionally, so a freshly-rebuilt bundle looks exactly as comment-free as a stale one. Check the `?<hash>` query string on the file (see cache-busting entry below) or diff actual logic instead.
...
- **Cache-busting on bundle URLs already works via `webassets`' defaults — no `url_expire` config needed**: `asset_env = Environment()` (`assets.py`) never explicitly sets `url_expire`, but `webassets==2.0`'s own defaults (`url_expire=None`, `versions='hash'`, no `%(version)s` placeholder in any `output=` path) combine to append a `?<hash>` query string to every bundle URL automatically — `Bundle._urls()` appends the version query string whenever `ctx.url_expire or (ctx.url_expire is None and not has_placeholder(self.output))`, which is true here. Confirmed live 2026-08-13: `frontendroute.js?d69b2007` became `frontendroute.js?110ec7bb` after `flask assets build` picked up a source change, with no explicit `Environment` config change needed. (An earlier note here claimed this was unconfigured/broken — it was checked against the wrong assumption about webassets' defaults, not against the actually-installed version.) Browsers must still be told to actually re-request `gen/*.js` when the *page* HTML itself is stale-cached (see the async Google Maps race below for an unrelated case of "looks stale after navigating").
...
- **Google Maps JS API loads async, gated by `onGmapsReady`**: The Maps script is *not* in an `assets.py` bundle (a bundled `<script>` can't carry the `async` attribute or `loading=async` param cleanly). Instead each of the three map-using templates (`frontend_routes.jinja2`, `frontend_route.jinja2`, `frontend_locations.jinja2`) has, in its `prescripts` block, an inline `<script>` stub defining `window.gmapsCallback`/`window.onGmapsReady(fn)` *followed by* `<script async src="...&loading=async&callback=gmapsCallback">`. The corresponding JS file (`runningroutes.js`, `runningroute-route.js`, `iconmap.js`) registers its map-dependent init via `window.onGmapsReady(fn)` and gates it behind two flags (`_domReady`, `_pageGmapsReady`) so init runs once, whichever finishes last. **Gotcha 1 (why the stub must be inline, not in the JS bundle):** the shared `frontendcommon.js` bundle is ~2MB and can take longer to download/parse/execute than the tiny async Maps bootstrap script takes to fetch its library and invoke the callback — defining `window.gmapsCallback` directly inside the page's own bundle (which loads after `frontendcommon.js`) lost that race in practice, throwing `InvalidValueError: gmapsCallback is not a function`. The inline stub runs before the async script tag is even requested, so `onGmapsReady` is always ready to queue the page's callback regardless of bundle load time. **Gotcha 2:** `runningroutes.js`/`runningroute-route.js` build `SVGOverlay` by extending `google.maps.OverlayView`; do this with `Object.setPrototypeOf(SVGOverlay.prototype, google.maps.OverlayView.prototype)` inside the gated callback, never `SVGOverlay.prototype = new google.maps.OverlayView()` — the latter, run after the file's own top-level `SVGOverlay.prototype.onIdle = ...`-style assignments (which run unconditionally at parse time), *replaces* the prototype object and silently wipes those methods out. **Gotcha 3 (`setelev`/`overlay.dist()` race, fixed 2026-08-13):** in `runningroute-route.js`, `SVGOverlay.prototype.setdata()` used to only compute the distance array (`this._dist`, read by `overlay.dist()`) as a side effect of `addmarkers()`, and only called `addmarkers()` if `this.svg` already existed — i.e. only if Maps' async `onAdd()` had already fired. `onAdd()` and the route-data AJAX response race independently of each other and of the Maps-ready gating above; when the AJAX response won that race (common after the async-loading change, since `onAdd()` now often fires later relative to page load than it used to under blocking script loading), `this._dist` stayed `undefined` and the immediately-following `setelev(data.path, overlay.dist())` call threw `TypeError: Cannot read properties of undefined (reading 'length')`, leaving the map visible but the elevation chart missing. Fixed by extracting the pure distance-array math into `SVGOverlay.prototype.computedist()` and calling it unconditionally from `setdata()` (before the `if (!this.svg) return` guard), so `overlay.dist()` is always valid immediately after `setdata()` returns regardless of `onAdd()` timing; `addmarkers()` now reads the already-computed `this._dist` instead of recomputing it. **Gotcha 4 (page-local ready flag must not be named `_gmapsReady`, fixed 2026-08-13):** each page JS file's own top-level `var _gmapsReady = false;` is a classic-script global `var`, which makes it a property of `window` — i.e. the *same* `window._gmapsReady` the inline template stub sets to `true` when `gmapsCallback` fires. When the Maps bootstrap script and its library fetch are both served from browser HTTP cache, `gmapsCallback` can fire (setting `window._gmapsReady = true`) *before* the page's own (larger, later-in-document) JS bundle has finished executing; when that bundle then runs its own top-level `var _gmapsReady = false;`, it silently stomps the already-true flag back to `false` — and since Google only invokes the callback once, the page hangs forever waiting on a callback that already fired. 100% reproducible once Maps is cache-served (which a plain reload/re-navigation does not clear). Fixed by renaming the page-local flag to `_pageGmapsReady` in all three JS files so it can't alias the stub's global. **Testing note:** a hard-refresh (DevTools "Empty Cache and Hard Reload" / Ctrl+Shift+R) bypasses HTTP cache for every resource including the Maps script, which forces the slow (network-fetch) path and can mask cache-dependent races like Gotcha 4 entirely. When testing changes to this async-loading pattern, also test a plain reload / fresh tab / direct URL nav (not just hard-refresh) so Maps gets a chance to resolve from cache and actually exercise the fast path.
- **`git commit` can report a tool timeout while the commit actually succeeds**: seen live 2026-08-14 — `git commit -F -` (heredoc via stdin) hung past a 2-minute timeout, and even `git commit -F <tmpfile>` hung past 30s; in both cases the commit had actually completed (`git log`/`git status` confirmed it immediately after, working tree clean). No pre-commit hooks are configured in this repo (`.git/hooks/` has no non-sample scripts) and `commit.gpgsign` isn't set, so it isn't hook or signing related — cause otherwise unconfirmed. **How to apply:** if a `git commit` call in this repo reports a timeout, check `git log -1`/`git status` before retrying or assuming failure — retrying a commit that actually succeeded creates a duplicate.
