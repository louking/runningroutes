...
- **Google Maps JS API loads async, gated by callback**: The Maps script is *not* in an `assets.py` bundle (a bundled `<script>` can't carry the `async` attribute or `loading=async` param cleanly). Instead each of the three map-using templates (`frontend_routes.jinja2`, `frontend_route.jinja2`, `frontend_locations.jinja2`) has its own `<script async src="https://maps.google.com/maps/api/js?key={{ config.GMAPS_API_KEY }}&loading=async&callback=gmapsCallback">` in its `prescripts` block. The corresponding JS file (`runningroutes.js`, `runningroute-route.js`, `iconmap.js`) defines `window.gmapsCallback` and gates its map-dependent init behind two flags (`_domReady`, `_gmapsReady`) so init runs once, whichever finishes last. **Gotcha:** `runningroutes.js`/`runningroute-route.js` build `SVGOverlay` by extending `google.maps.OverlayView`; do this with `Object.setPrototypeOf(SVGOverlay.prototype, google.maps.OverlayView.prototype)` inside the gated callback, never `SVGOverlay.prototype = new google.maps.OverlayView()` — the latter, run after the file's own top-level `SVGOverlay.prototype.onIdle = ...`-style assignments (which run unconditionally at parse time), *replaces* the prototype object and silently wipes those methods out.
